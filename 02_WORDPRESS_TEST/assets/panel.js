...
(function ($) {
  ...
  // CORRECCIÓN 1: nombres y colores corregidos según design system y WordPress
  const PILARES_DEF = [
    { code: 'P01', nombre: 'Rufino real',             color: '#c0271b', slug: 'rufino-real' },
    { code: 'P02', nombre: 'El campo habla',           color: '#617a45', slug: 'el-campo-habla' },
    { code: 'P03', nombre: 'Barrio a barrio',          color: '#b55233', slug: 'barrio-a-barrio' },
    { code: 'P04', nombre: 'Generación Rufino',        color: '#c58a2b', slug: 'generacion-rufino' },
    { code: 'P05', nombre: 'Seguimiento de promesas',  color: '#1f2a30', slug: 'seguimiento-promesas' },
    { code: 'P06', nombre: 'Contexto y datos',         color: '#2f6484', slug: 'contexto-datos' },
  ];
  ...
  function renderWizard() {
          ...
          <div class="er-wizard-card">
            ...
            <div class="er-wizard-body">
              ...
              <div class="er-wizard-nav">
                ${step > 1 ? `<button class="er-btn er-btn-ghost er-btn-sm" id="w-prev">← Anterior</button>` : `<span></span>`}
                ${step < 3 ? `<button class="er-btn er-btn-rojo" id="w-next">Siguiente →</button>` : `<button class="er-btn er-btn-rojo" id="w-finish">✓ Finalizar configuración</button>`}
              </div>
            </div>
          </div>
        </div>
    ...
    function bindWizardEvents() {
      ...
      $('#w-next').on('click', () => { if (!collectStep()) return; step++; renderStep(); });
      ...
      $('#w-finish').on('click', () => {
        ...
        $.post(ajaxUrl, {
          action: 'er_save_wizard', nonce,
          claim: data.claim, color: data.color,
          api_key: data.api_key, pilares: data.pilares,
        }, res => {
          if (res.success) { window.location.href = res.data.redirect; }
          else { $btn.text('✓ Finalizar configuración').prop('disabled', false); alert('Error al guardar. Intentá de nuevo.'); }
        });
      });
    }
  ...
  function renderAdminHome() {
    ...
    `;
    $('.er-module-card').on('click', function () { loadModule($(this).data('module')); });
  }
        ...
        <div class="er-produccion-main">
          ...
          <div class="er-form-group">
            ...
            <div style="position:relative">
              <textarea class="er-textarea" id="p-transcripcion" rows="5" placeholder="Pegá citas textuales, puntos clave, datos..."></textarea>
  ...
  function bindProduccion() {
    ...
    $('#p-auto-trans').on('click', function () {
      ...
      $.post(ajaxUrl, { action: 'er_yt_captions', nonce, video_id: id }, res => {
        ...
        if (res.success) { $('#p-transcripcion').val(res.data.transcripcion); }
        else { showAlert('#p-error', res.data?.msg || 'Sin subtítulos disponibles', 'warn'); }
      });
    });
    ...
    $('#p-generar').on('click', function () {
      ...
      $.post(ajaxUrl, {
        action: 'er_asistente_generar', nonce, titulo,
        pilar: $('#p-pilar').val(),
        youtube: $('#p-yt').val(),
        ...
        if (res.success) { $('#p-output').addClass('visible').text(res.data.content); $('#p-output-actions').show(); }
        else { showAlert('#p-error', res.data?.msg || 'Error al generar', 'err'); }
      });
    });
    ...
    $('#p-guardar').on('click', function () {
      ...
      $.post(ajaxUrl, {
        action: 'er_guardar_borrador', nonce,
        titulo: $('#p-titulo').val(), cuerpo: $('#p-output').text(),
        categoria: $('#p-cat').val(), tags: $('#p-tags').val(),
      }, res => {
        $(this).text('💾 Guardar como borrador').prop('disabled', false);
        if (res.success) { showAlert('#p-guardado', `✓ Borrador guardado — <a href="${res.data.edit_url}" target="_blank">Editar en WordPress</a>`, 'ok'); }
        else { showAlert('#p-guardado', res.data?.msg || 'Error al guardar', 'err'); }
      });
    });

    $('#p-copiar').on('click', function () {
      navigator.clipboard.writeText($('#p-output').text()).then(() => {
        $(this).text('¡Copiado!');
    ...
    $('#s-prom-guardar').on('click', function () {
      ...
      $(this).text('...').prop('disabled', true);
      $.post(ajaxUrl, {
        action: 'er_save_promesa', nonce, texto,
        ...
        if (res.success) {
          ...
          $('#s-prom-texto').val(''); $('#s-prom-fuente').val('');
          loadPromesamsMini();
        }
      });
    });

    $('#s-buscar-btn').on('click', () => {
      const q = $('#s-buscar').val().trim();
      if (!q) return;
      $('#s-resultado').html('<div class="er-spinner" style="margin:0"></div>');
      $.post(ajaxUrl, { action: 'er_buscar_contexto', nonce, query: q }, res => {
        if (res.success && res.data.length) {
          $('#s-resultado').html(res.data.map(r =>
            `<div style="padding:6px 0;border-bottom:1px solid var(--borde);font-size:12px">
              <a href="${r.url}" target="_blank" style="font-weight:600;color:var(--negro)">${escHtml(r.titulo)}</a>
              <div style="color:var(--gris)">${escHtml(r.excerpt)}</div>
            </div>`).join(''));
        } else {
          $('#s-resultado').html('<span style="color:var(--gris);font-size:12px">Sin resultados.</span>');
        }
      });
    });
  }
  ...
  function loadSeguimiento() {
    ...
    function renderTabla() {
      const filtradas = filtroActivo === 'todos' ? allPromesas : allPromesas.filter(p => p.estado === filtroActivo);
      if (!filtradas.length) {
        $('#s-tabla-wrap').html(`
      ...
      const rows = filtradas.map(p => `
        <tr>
          ...
          <td><button class="er-btn er-btn-ghost er-btn-sm s-del" data-id="${p.id}" title="Eliminar">✕</button></td>
        </tr>
      `).join('');
      $('#s-tabla-wrap').html(`
        <div class="er-table-wrap">
          <table class="er-table">
            <thead><tr><th>Promesa</th><th>Fuente</th><th>Fecha</th><th>Pilar</th><th>Estado</th><th>Cambiar</th><th></th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>
      ...
      $('.er-estado-select').on('change', function () {
        const id = $(this).data('id'), estado = $(this).val();
        $.post(ajaxUrl, { action: 'er_update_promesa', nonce, id, estado }, res => {
          if (res.success) { const idx = allPromesas.findIndex(p => p.id === id); if (idx !== -1) allPromesas[idx].estado = estado; renderTabla(); }
        });
      });
      ...
      $('.s-del').on('click', function () {
        ...
        $.post(ajaxUrl, { action: 'er_delete_promesa', nonce, id }, res => {
          if (res.success) { allPromesas = allPromesas.filter(p => p.id !== id); renderTabla(); }
        });
      });
    }
    ...
    $('#s-export').on('click', () => {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'promesas-el-rufino.csv'; a.click();
  ...
  function loadBorradores() {
    ...
    $.post(ajaxUrl, { action: 'er_get_borradores', nonce }, res => {
      ...
      if (!borradores.length) {
        $('#b-lista').html(`<div class="er-empty-state"><div class="er-empty-state-icon">📝</div><h3>Sin borradores aún</h3><p>Las notas guardadas desde Producción van a aparecer acá.</p></div>`);
        return;
      }
      ...
      `).join('');
      $('#b-lista').html(`<div class="er-table-wrap"><table class="er-table"><thead><tr><th>Título</th><th>Categoría</th><th>Modificado</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`);
    });
  }
  ...
  function loadInteligencia() {
    root.innerHTML = `
      ...
      <div style="padding:32px;max-width:700px">
        <p style="color:var(--gris);margin-bottom:24px;font-size:14px">La Inteligencia está integrada como sidebar en el módulo de Producción. Desde ahí podés buscar contexto local y registrar promesas mientras redactás.</p>
        <button class="er-btn er-btn-rojo" onclick="loadModule('produccion')">Ir a Producción →</button>
    ...
    $('#i-buscar-btn').on('click', () => {
      ...
      $.post(ajaxUrl, { action: 'er_buscar_contexto', nonce, query: q }, res => {
        if (res.success && res.data.length) {
          $('#i-resultado').html(res.data.map(r =>
      ...
      <div class="er-dashboard-wrap">
        <div class="er-dashboard-section">
          ...
          <div class="er-form-group">
            ...
            <div class="er-key-status" id="d-key-status">
              ${keyOk ? `<span class="er-key-ok">✓ Configurada — ${escHtml(keyMasked)}</span>` : `<span class="er-key-no">✗ Sin configurar</span>`}
            </div>
          </div>
          <div class="er-form-group">
        ...
        <div class="er-dashboard-section">
          ...
          <table style="width:100%;font-size:13px">
            ...
            <tr><td style="padding:6px 0;color:var(--gris)">Modelo IA</td><td><strong>claude-sonnet-4-20250514</strong></td></tr>
    ...
    $('#d-save').on('click', function () {
      ...
      $.post(ajaxUrl, { action: 'er_save_key', nonce, key }, res => {
        ...
        if (res.success) {
          ...
          $('#d-key').val('');
        } else { showAlert('#d-alert', 'Error al guardar', 'err'); }
      });
    });
  }
  ...
  function headerHtml(subtitulo, showBack) {
    return `
      <div class="er-header">
        ...
        <div class="er-header-actions">
          ${showBack ? `<button class="er-btn er-btn-outline er-btn-sm" onclick="loadModule('home')">← Panel</button>` : ''}
  ...
  function showAlert(selector, msg, type) {
    const $el = $(selector);
    $el.removeClass('er-alert-ok er-alert-err er-alert-warn').addClass(`er-alert-${type} visible`).html(msg);
