...
// ── Reemplazar footer de Newsup con footer propio ──
add_action('init', 'er_remove_newsup_footer');
function er_remove_newsup_footer() {
    remove_action('newsup_footer', 'newsup_footer_content');
    remove_action('wp_footer', 'newsup_footer_content');
}

add_action('wp_footer', 'er_render_footer', 1);
$GLOBALS['er_footer_rendered'] = false;
function er_render_footer() {
    if (!empty($GLOBALS['er_footer_rendered'])) return;
    $GLOBALS['er_footer_rendered'] = true;
        ...
        <div class="er-footer-grid">
          ...
          <div>
            ...
            <ul class="er-footer-links">
              <?php foreach ($pilares as $slug => $nombre) :
