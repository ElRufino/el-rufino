...
function er_crear_categorias() {
    $pilares = [
        ...
        'Contexto y datos'        => ['slug' => 'contexto-datos',         'color' => '#2f6484'],
        'El campo habla'          => ['slug' => 'el-campo-habla',         'color' => '#617a45'],
        'Generación Rufino'       => ['slug' => 'generacion-rufino',      'color' => '#c58a2b'],
        'Seguimiento de promesas' => ['slug' => 'seguimiento-promesas',   'color' => '#1f2a30'],
    ];
...
add_action('wp_ajax_er_get_checklist', function () {
    check_ajax_referer('er_nonce', 'nonce');
    $default = [
        ['id' => 1, 'texto' => '6 categorías P01-P06',             'ok' => false],
        ['id' => 2, 'texto' => 'Logo/favicon/OG subidos',          'ok' => false],
        ['id' => 3, 'texto' => 'Schema NewsMediaOrganization',     'ok' => false],
        ['id' => 4, 'texto' => '20 notas publicadas',              'ok' => false],
        ['id' => 5, 'texto' => 'Child theme Newsup instalado',     'ok' => false],
        ['id' => 6, 'texto' => 'Dominio .ar registrado en NIC',    'ok' => false],
        ['id' => 7, 'texto' => 'Canal WhatsApp creado',            'ok' => false],
        ['id' => 8, 'texto' => 'Plugin v8.5 en producción',        'ok' => false],
    ];
...
/* ═══════════════════════════════════════════════════════════
   7. AJAX — PROXY IA (Anthropic directo · OpenRouter)
═══════════════════════════════════════════════════════════ */
add_action('wp_ajax_er_claude', function () {
    ...
    // Autodetección: usa er_ai_provider si está guardado,
    // si no, detecta por prefijo de key (compatibilidad con versiones anteriores).
    ...
    // Si guardaron una key OR en el campo de Anthropic, usarla igual
    if ($provider === 'anthropic' && str_starts_with($ant_key, 'sk-or-')) {
        $provider = 'openrouter';
        if (!$or_key) { update_option('er_or_api_key', $ant_key); $or_key = $ant_key; }
    }
    ...
    if ($provider === 'openrouter') {
        ...
        if (!$api_key) {
            wp_send_json_error(['msg' => 'API key de OpenRouter no configurada. Configurala en Dashboard → Proveedor IA.']);
            return;
        }
        $model = get_option('er_or_model', 'anthropic/claude-sonnet-4');

        // Convertir al formato OpenAI-compatible (system como primer mensaje)
        $or_messages = [];
        if ($system) {
        ...
        if (is_wp_error($response)) {
            wp_send_json_error(['msg' => 'Error de conexión con OpenRouter: ' . $response->get_error_message()]);
            return;
        }
        $data = json_decode(wp_remote_retrieve_body($response), true);
        if (!empty($data['error'])) {
            wp_send_json_error(['msg' => 'Error OpenRouter: ' . ($data['error']['message'] ?? json_encode($data['error']))]);
            return;
        }
        wp_send_json_success(['content' => $data['choices'][0]['message']['content'] ?? '']);
        return;
    }
...
/* — Guardar key OpenRouter — */
add_action('wp_ajax_er_save_orkey', function () {
    check_ajax_referer('er_nonce', 'nonce');
    $key   = trim(sanitize_text_field($_POST['key']   ?? ''));
    $model = trim(sanitize_text_field($_POST['model'] ?? 'anthropic/claude-sonnet-4'));
    update_option('er_or_api_key', $key);
    update_option('er_or_model',   $model ?: 'anthropic/claude-sonnet-4');
    wp_send_json_success(['saved' => true]);
});

/* — Estado key OpenRouter — */
add_action('wp_ajax_er_orkey_status', function () {
    check_ajax_referer('er_nonce', 'nonce');
    $key   = get_option('er_or_api_key', '');
    $model = get_option('er_or_model', 'anthropic/claude-sonnet-4');
    wp_send_json_success([
        'configured' => !empty($key),
        'masked'     => $key ? substr($key, 0, 12) . '...' : '',
        'model'      => $model,
    ]);
});

/* — Cambiar proveedor activo — */
add_action('wp_ajax_er_save_provider', function () {
    check_ajax_referer('er_nonce', 'nonce');
    $provider = sanitize_text_field($_POST['provider'] ?? 'anthropic');
    if (!in_array($provider, ['anthropic', 'openrouter'], true)) $provider = 'anthropic';
    update_option('er_ai_provider', $provider);
    wp_send_json_success(['provider' => $provider]);
});
...
function er_notas_demo() {
    return [
        ...
        ['titulo'=>'Cómo creció la población de Rufino en la última década','pilar'=>'contexto-datos',      'prompt'=>'Escribí una nota con datos sobre el crecimiento poblacional de Rufino, Santa Fe. Según el INDEC 2022 son 19.211 habitantes. Analizá qué significa ese crecimiento para los servicios locales. Incluí: TÍTULO, BAJADA, CUERPO (3 párrafos con datos), LO QUE SIGNIFICA. Máximo 400 palabras.'],
        ['titulo'=>'Estadísticas de empleo en Rufino: primer semestre 2026','pilar'=>'contexto-datos',      'prompt'=>'Escribí una nota de contexto sobre el empleo en Rufino y el sur de Santa Fe en el primer semestre 2026. Referenciá datos del INDEC y del mercado laboral regional. Incluí: TÍTULO, BAJADA, CUERPO (3 párrafos), LO QUE SIGNIFICA. Máximo 400 palabras.'],
        ...
        ['titulo'=>'El municipio prometió pavimentar tres calles del sur',  'pilar'=>'seguimiento-promesas','prompt'=>'Escribí una nota de seguimiento: el intendente de Rufino prometió pavimentar tres calles del barrio sur en el presupuesto 2026. Ya pasaron 4 meses y no empezaron. Incluí: TÍTULO, BAJADA, CUERPO (3 párrafos con seguimiento), LO QUE SIGNIFICA. Máximo 400 palabras.'],
        ['titulo'=>'Semáforo de promesas: qué se cumplió y qué no en 2025','pilar'=>'seguimiento-promesas','prompt'=>'Escribí una nota de balance: repasá 5 promesas del gobierno municipal de Rufino en 2025 y evaluá su estado (cumplida, parcial, incumplida). Usá el formato "semáforo". Incluí: TÍTULO, BAJADA, CUERPO (lista de promesas con estado), LO QUE SIGNIFICA. Máximo 450 palabras.'],
    ];
}
...
function er_unsplash_query_for_pilar($pilar) {
    $map = [
        ...
        'contexto-datos'       => 'data statistics chart argentina',
        'el-campo-habla'       => 'soybean farm argentina campo',
        'generacion-rufino'    => 'youth sport argentina students',
        'seguimiento-promesas' => 'city hall government argentina',
    ];
...
add_action('wp_ajax_er_import_demo', function () {
    ...
    $notas_demo = [
        ...
        [
            ...
            'pilar'  => 'contexto-datos',
            'prompt' => 'Escribí una nota de contexto y datos sobre el empleo en Rufino y el sur de Santa Fe en el primer semestre 2026. '
        ...
        [
            ...
            'pilar'  => 'seguimiento-promesas',
            'prompt' => 'Escribí una nota de seguimiento de promesas: el intendente de Rufino prometió pavimentar tres calles '
...
SYSTEM;
    ...
    $response = wp_remote_post('https://api.anthropic.com/v1/messages', [
        'timeout' => 120,
        'headers' => ['x-api-key' => $api_key, 'anthropic-version' => '2023-06-01', 'content-type' => 'application/json'],
        'body'    => wp_json_encode(['model' => 'claude-sonnet-4-20250514', 'max_tokens' => 2000, 'system' => $system, 'messages' => [['role' => 'user', 'content' => $message_content]]]),
    ]);
    if (is_wp_error($response)) {
...
function er_widget_pilares() {
    $pilares = [
        ...
        ['Contexto y datos',        'contexto-datos',         '#2f6484'],
        ['El campo habla',          'el-campo-habla',         '#617a45'],
        ['Generación Rufino',       'generacion-rufino',      '#c58a2b'],
        ['Seguimiento de promesas', 'seguimiento-promesas',   '#1f2a30'],
    ];
    $max = 1;
    $counts = [];
...
function er_widget_ultimas() {
    $pilares_color = [
        ...
        'contexto-datos'         => '#2f6484',
        'el-campo-habla'         => '#617a45',
        'generacion-rufino'      => '#c58a2b',
        'seguimiento-promesas'   => '#1f2a30',
    ];
